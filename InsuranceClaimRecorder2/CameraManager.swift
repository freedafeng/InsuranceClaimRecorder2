...
class CameraManager: NSObject, ObservableObject, AVCaptureFileOutputRecordingDelegate, CLLocationManagerDelegate {
    ...
    func uploadVideo(from url: URL, metadata: [String: Any]?) {
        ...
        if let videoData = try? Data(contentsOf: url) {
            ...
            data.append("Content-Disposition: form-data; name=\"file\"; filename=\"claim_video.mov\"\r\n".data(using: .utf8)!)
        ...
        // Add metadata
        if let metadata = metadata, let metadataJson = try? JSONSerialization.data(withJSONObject: metadata, options: .prettyPrinted) {
            data.append("--\(boundary)\r\n".data(using: .utf8)!)
